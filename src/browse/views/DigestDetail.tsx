  ...
  return (
    <Box flexDirection="column">
      {windowLines.map((line, i) => {
        ...
        if (line.type === "header") {
          return (
            <Text key={i} bold color={(line.color || "white") as any}>
              {line.text}
            </Text>
          );
        }
        if (line.type === "agent") {
          return (
            <Text key={i} color={(line.color || "white") as any}>
              {line.text}
            </Text>
          );
        }
      ...
      <StatusBar view="detail" searchActive={false} info="^/v Scroll  esc Back  q Quit" />
    </Box>
  );
}
...
function buildLines(report: DigestReport): DisplayLine[] {
  ...
  lines.push({
    type: "header",
    text: `Digest: ${report.window.since.slice(0, 10)} → ${report.window.until.slice(0, 10)}`,
    color: "white",
  });
  lines.push({
    type: "item",
    text: `${report.totalSessions} sessions | ${report.totalPrompts} prompts | ${report.scope}`,
    dimColor: true,
  });
  ...
  lines.push({
    type: "item",
    text: `  Input: ${formatTokens(t.totalInput)}  Output: ${formatTokens(t.totalOutput)}  Cache: ${formatTokens(t.totalCacheRead)}  API: ${t.totalApiCalls}`,
  });

  for (const [agent, data] of Object.entries(t.byAgent)) {
    lines.push({
      type: "agent",
      text: `  ${agent}: ${formatTokens(data.input + data.output)} tok, ${data.sessionCount}s, ${data.apiCalls} calls`,
      color:
        agent === "claude"
          ? "blue"
          : agent === "codex"
  ...
  if (topProjects.length > 0) {
    ...
    for (const [name, data] of topProjects) {
      lines.push({
        type: "item",
        text: `  ${name}: ${formatTokens(data.input + data.output)} (${data.sessionCount}s)`,
      });
    }
    lines.push({ type: "empty", text: "" });
  }
  ...
  for (const cat of cats) {
    ...
    lines.push({
      type: "header",
      text: `${CATEGORY_LABELS[cat]} (${items.length})`,
    ...
    if (items.length === 0) {
      ...
      for (const item of items.slice(0, 10)) {
        lines.push({
          type: "item",
          text: `  ${String(item.count).padStart(3)}x  ${item.description.slice(0, 60)}`,
        });
        lines.push({
          type: "item",
          text: `       [${item.agents.join(",")}] ${item.projects.join(", ")}`,
          dimColor: true,
        });
      }
      if (items.length > 10) {
  ...
  if (report.uncategorized.length > 0) {
    lines.push({
      type: "header",
      text: `UNCATEGORIZED (${report.uncategorized.length})`,
      color: "white",
    });
