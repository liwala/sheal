...
1. **Raw sessions** — immutable episodic material (transcripts, retros, git
   diffs). Sheal reads but never owns these.
...
- Q5. Cross-agent provenance — how does sheal attribute a learning to "Claude
  in session X" vs. "Codex in session Y"? (Affects how skills compile back.)
